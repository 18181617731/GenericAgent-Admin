...
def cell_len(value: str) -> int:
    return sum(_cell_width(char) for char in value)


def set_cell_size(value: str, max_cells: int) -> str:
    out = []
    width = 0
    for char in value:
        next_width = width + _cell_width(char)
        if next_width > max_cells:
            break
        out.append(char)
        width = next_width
    return "".join(out)
...
class RewindStore:
    ...
    @staticmethod
    def _strip_project_mode(text: str) -> str:
        return re.sub(r"\n*-{3,}\n\[PROJECT MODE:.*?\n-{3,}\s*$", "", text or "", flags=re.DOTALL)
    ...
    def commit(self, title: str, hist_len: int | None = None, kind: str = "edit",
        ...
        self.nodes[nid] = {
            ...
            "files": files,
            "conv": conv,
            "hist_info": list(hist_info) if isinstance(hist_info, list) else None,
...
def restore_plan(store, node_id, mode: str = "both", to: str = "before",
    ...
    target_node = store.nodes[logical_target]
    hist_info = target_node.get("hist_info")
    key_info = target_node.get("key_info")
    history = None
    if mode in ("both", "conv"):
        history = store.rebuild_history(logical_target)
    changed = []
    if mode in ("both", "code"):
        try:
            changed = store.apply_code(logical_target)
        except Exception:
            changed = []
    # Conversation restores use a physical bridge beside the selected logical
    # node. A subsequent commit can then fork without mutating the original
    # branch. Admin's sidecar folds this bridge back into logical_target for UI.
    if mode in ("both", "conv") and not at_origin:
        selected = store.nodes[logical_target]
        parent = selected.get("parent")
        parent = parent if parent in store.nodes else store.root_id
        store.rewind_head(parent)
        target = store.commit(
            selected.get("title") or "恢复点",
            kind="rewind",
            history=list(history or []),
            hist_info=list(hist_info) if isinstance(hist_info, list) else None,
            key_info=key_info if isinstance(key_info, str) else None,
        )
        store.nodes[target]["files"] = dict(selected.get("files") or {})
        store.nodes[target]["rw_tag"] = logical_target
        store.save()
    else:
        store.rewind_head(logical_target)
