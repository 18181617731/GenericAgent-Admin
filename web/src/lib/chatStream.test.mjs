...
test('idle session summary changes request an authoritative snapshot refresh', () => {
  ...
  const isoBase = { ...base, updated_at:'2026-07-17T13:00:00Z' }
  assert.equal(shouldRefreshChatSnapshot(isoBase, { ...isoBase }), false)
  assert.equal(shouldRefreshChatSnapshot(isoBase, { ...isoBase, updated_at:'2026-07-17T13:00:01Z' }), true)
