...
test('version update UI keeps destructive update behind status-aware controls', () => {
  ...
  assert.match(app, /disabled=\{versionBusy \|\| versionStatus\?\.running \|\| \(!versionCheck\?\.update && !versionUX\.canRetry\)\}/)
  assert.match(app, /versionUX\.actionLabel/)
  assert.match(app, /setInterval\(\(\) => refreshVersionStatus\(\)\.catch\(e => setMsg\(e\.message\)\), 1500\)/)
  assert.match(app, /api\('\/api\/version\/status'\)/)
  assert.match(app, /api\('\/api\/version\/info'\)\.catch\(\(\) => null\)/)
  assert.match(app, /d\.stage === 'done'/)
  assert.match(app, /versionStatus\.applied_version/)
