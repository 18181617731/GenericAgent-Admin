const COPY = {
  zh: {
    ...
    pending: '待处理', handled: '已处理', noPending: '当前没有待审批任务', noHandled: '还没有审批记录', noLedger: '未发现待批草案台账', handledGroupsHelp: '已处理记录按审批状态分类，点击分类标题展开或收起。', handledGroups: { approved: { label: '已批准', description: '用户已批准，正在排队或已进入执行流程' }, rejected: { label: '已拒绝', description: '用户已拒绝，不会进入自主任务队列' }, archived: { label: '已归档', description: '无法归入标准审批状态的历史记录' } },
  ...
  en: {
    ...
    pending: 'Pending', handled: 'Handled', noPending: 'No tasks are waiting for approval', noHandled: 'No approval decisions yet', noLedger: 'No pending-draft ledger was found', handledGroupsHelp: 'Handled records are grouped by approval status. Select a group header to expand or collapse it.', handledGroups: { approved: { label: 'Approved', description: 'Approved by the user; queued or moving through execution' }, rejected: { label: 'Rejected', description: 'Rejected by the user; it will not enter the autonomous queue' }, archived: { label: 'Archived', description: 'Historical records without a standard approval status' } },
