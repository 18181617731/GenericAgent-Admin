...
test('project new chat is always available outside the actions menu', () => {
  const list = readFileSync(new URL('../components/ChatSessionList.jsx', import.meta.url), 'utf8')
  const header = functionBlock(list, '<div className="oa-project-head">', '<div className="oa-project-body"')
  ...
  assert.match(outsideMenu, /onClick=\{\(\)\s*=>\s*onNewProjectSession\?\.\(group\.provider\s*\?\s*group\s*:\s*group\.name\)\}/)
  assert.match(outsideMenu, /disabled=\{batchDeleting\}/)
  assert.doesNotMatch(header.slice(menuStart), /oa-project-add/)
  assert.doesNotMatch(list, /No chats yet\. Start one from the/)
})

test('pinned projects expose a persistent status outside the menu and collapsed body', () => {
  const list = readFileSync(new URL('../components/ChatSessionList.jsx', import.meta.url), 'utf8')
  const header = functionBlock(list, '<div className="oa-project-head">', '<ProjectActionsMenu')
  ...
  assert.match(css, /\.oa-sidebar \.oa-project-pinned-badge\s*\{[^}]*display\s*:\s*inline-flex;[^}]*flex\s*:\s*0 0 auto;/)
})
...
test('project drag handles are opt-in from the new-project toolbar', () => {
  const main = readFileSync(new URL('../ChatApp.jsx', import.meta.url), 'utf8')
  const sidebar = readFileSync(new URL('../components/ChatSidebar.jsx', import.meta.url), 'utf8')
  const list = readFileSync(new URL('../components/ChatSessionList.jsx', import.meta.url), 'utf8')
  assert.match(main, /\[projectSortMode, setProjectSortMode\] = useState\(false\)/)
  assert.match(main, /const saveProjectOrder = async names =>/)
  const toolbar = functionBlock(sidebar, '<div className="oa-session-manager-head">', '<ChatSessionList')
  assert.match(toolbar, /aria-pressed=\{projectSortMode\}/)
  assert.match(toolbar, /onProjectSortModeChange\?\.\(!projectSortMode\)/)
  assert.match(toolbar, /onClick=\{onOpenProjectDraft\}/)
  assert.match(sidebar, /projectSessionGroups\.length < 2/)
  assert.match(sidebar, /projectSortMode=\{projectSortMode\}/)
  assert.match(sidebar, /onProjectOrder=\{onProjectOrder\}/)
  assert.match(main, /setProjectSortMode\(false\)/)
  const header = functionBlock(list, '<div className="oa-project-head">', '<div className="oa-project-body"')
  assert.match(header, /\{projectSortMode\s*&&\s*\(/)
  assert.match(header, /<ProjectDragHandle[\s\S]*onReorder=\{onProjectOrder\}/)
})
...
test('project folder action uses selected instance and confirmed folder opening', () => {
  ...
  assert.ok(handler.includes("project_provider: project.provider || 'official', project_id: project.id || name"))
  assert.match(handler, /mode: 'folder'/)
  assert.match(list, /onClick=\{\(\)\s*=>\s*onOpenProjectFolder\?\.\(group\)\}/)
  assert.match(source, /onOpenProjectFolder=\{openProjectFolder\}/)
