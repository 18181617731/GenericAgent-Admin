export const I18N = {
  zh: {
    ...
    tasks: { form: '表单编辑', json: 'JSON 编辑', formHelp: '表单编辑定时任务核心字段，修改实时同步到 JSON；保存/删除会生成 .bak 时间戳。', parseFailed: 'JSON 解析失败，回退到文本编辑', enabledLabel: 'enabled（是否启用定时任务）', maxDelay: 'max_delay_hours（最大延迟小时数）', repeat: 'repeat（执行频率：每日/每周/每N小时/手动/单次）', choose: '-- 选择 --', schedule: 'schedule（定时执行时间，如 06:00）', schedulePlaceholder: '如 06:00', executionModel: '执行模型', defaultModel: '默认调度模型', schedulerDefaultLabel: 'GA 默认模型', executionModelRunning: '调度器运行中，请先停止后修改执行模型', executionModelHelp: '选择后，任务执行时会临时切换到该模型，完成后恢复调度器默认模型。', modelUnavailable: '已保存模型不可用，请重新选择', modelUnavailableHelp: '已保存模型不在当前服务商列表中，请重新选择并保存。', modelLegacyHelp: '此任务仍使用旧的模型序号；请确认当前模型后保存，以锁定模型。', unnamedProvider: '未命名服务商', unnamedModel: '未命名模型', prompt: 'prompt（定时任务的提示词内容）', promptPlaceholder: '定时任务的提示词内容', extraFields: '额外字段', unnamed: '未命名任务', unscheduled: '未排程', manual: '手动', explicitEnable: '需显式启用后才会运行', configError: '配置异常', overdue: '调度逾期', executionResult: '执行结果', configStatus: '配置状态', report: '报告', runNow: '立即执行', runPending: '执行中…', reportsAction: '执行记录', runConfirm: id => `立即执行任务 ${id}？这会启动一次独立执行，不会修改定时计划。`, runStarted: id => `任务 ${id} 已开始执行，完成后可在执行记录查看。`, runCompleted: id => `任务 ${id} 执行完成。`, runFailed: (id, error) => `任务 ${id} 执行失败${error ? `：${error}` : ''}` },
  ...
  en: {
    ...
    tasks: { form: 'Form editor', json: 'JSON editor', formHelp: 'Edit the core scheduled-task fields in a form. Changes sync to JSON; save/delete creates a timestamped .bak.', parseFailed: 'JSON parsing failed; using the text editor', enabledLabel: 'enabled (run this scheduled task)', maxDelay: 'max_delay_hours (maximum delay in hours)', repeat: 'repeat (daily, weekly, every N hours, manual, or once)', choose: '-- Select --', schedule: 'schedule (execution time, for example 06:00)', schedulePlaceholder: 'For example 06:00', executionModel: 'Execution model', defaultModel: 'Default scheduler model', schedulerDefaultLabel: 'GA default model', executionModelRunning: 'Stop the scheduler before changing its execution model', executionModelHelp: 'The task temporarily switches to this model and restores the scheduler default after it finishes.', modelUnavailable: 'Saved model unavailable; choose again', modelUnavailableHelp: 'The saved model is not in the current provider list. Choose it again and save.', modelLegacyHelp: 'This task still uses a legacy model number; confirm the current model and save to lock it.', unnamedProvider: 'Unnamed provider', unnamedModel: 'Unnamed model', prompt: 'prompt (scheduled-task instructions)', promptPlaceholder: 'Scheduled-task instructions', extraFields: 'Additional fields', unnamed: 'Unnamed task', unscheduled: 'Not scheduled', manual: 'Manual', explicitEnable: 'Explicitly enable this task before it can run', configError: 'Configuration error', overdue: 'Schedule overdue', executionResult: 'Execution result', configStatus: 'Configuration status', report: 'Report', runNow: 'Run now', runPending: 'Running…', reportsAction: 'Execution records', runConfirm: id => `Run task ${id} now? This starts one independent run and does not change its schedule.`, runStarted: id => `Task ${id} started. Its report will appear under Execution records.`, runCompleted: id => `Task ${id} completed.`, runFailed: (id, error) => `Task ${id} failed${error ? `: ${error}` : ''}` },
...
I18N.zh.tasks.modelDispatchUpdated = restarted => restarted ? '任务已保存，模型调度已启用并已重启调度器。' : '任务已保存，模型调度已启用；调度器下次启动时生效。'
I18N.en.tasks.modelDispatchUpdated = restarted => restarted ? 'Task saved. Model dispatch is enabled and the scheduler restarted.' : 'Task saved. Model dispatch is enabled for the next scheduler start.'
