...
import { Archive, Bot, ChevronLeft, Edit3, FolderPlus, GitFork, MessageSquarePlus, PanelLeftClose, Pin, Search, Settings, Trash2 } from 'lucide-react'
...
export default function ChatSidebar({
  ...
  return <>
    <aside ref={sidebarRef} id="oa-chat-sidebar" className={`oa-sidebar ${collapsed ? 'collapsed' : ''}`} aria-label={ct('聊天侧栏', 'Chat sidebar')} aria-hidden={collapsed || undefined}>
      ...
      <button className="oa-sidebar-search-action" type="button" onClick={onOpenSearch} ref={searchTriggerRef} title={searchLabel} aria-label={searchLabel} aria-haspopup="dialog"><Search size={16} aria-hidden="true"/><span>{searchLabel}</span><kbd>Ctrl/Cmd+K</kbd></button>
      ...
      <div className="oa-session-manager-head">
        ...
        <span className="oa-session-manager-actions">
          {sidebarTab === 'projects' && <button className="oa-session-manage-open" type="button" onClick={onOpenProjectDraft} disabled={projectCreating || projectDraftOpen}><FolderPlus size={13} aria-hidden="true"/>{ct('新建', 'New')}</button>}
      ...
      {sidebarTab === 'projects' && projectDraftOpen && <form className="oa-project-draft" onSubmit={event => { event.preventDefault(); onCreateProject?.() }}>
        <input
          autoFocus
          type="text"
      ...
      {!sessionManagerOpen && menuOpen && menuPos && menuSession && <div ref={menuRef} className="oa-session-menu" style={{ top: menuPos.top, left: menuPos.left }} role="menu" aria-label={ct('会话操作', 'Session actions')} aria-busy={menuActionBusy || undefined} onClick={event => event.stopPropagation()} onKeyDown={onMenuKeyDown}>
        <button type="button" role="menuitem" disabled={menuActionBusy} onClick={() => onStartRename?.(menuSession)}><Edit3 size={14} aria-hidden="true"/>{ct('重命名', 'Rename')}</button>
