...
function MetricStrip({ metrics, zh, activeFilter, onFilter }) {
  const attention = metrics.blocked + metrics.failed + metrics.overdue
  const tooltip = (zh ? ['待审批', '运行中', '阻塞/暂停', '失败', '逾期', '已完成'] : ['Approval', 'Active', 'Blocked', 'Failed', 'Overdue', 'Completed']).map((label, index) => `${label}: ${[metrics.pending, metrics.running, metrics.blocked, metrics.failed, metrics.overdue, metrics.completed][index]}`).join(' · ')
  const cards = [
    { key: 'pending', status: 'pending_approval', label: zh ? '待审批' : 'Approval', value: metrics.pending },
    { key: 'running', status: 'running', label: zh ? '运行中' : 'Active', value: metrics.running },
    { key: 'attention', status: '__attention', label: zh ? '需关注' : 'Needs attention', value: attention },
  ...
  return <div className="autonomous-task-metrics" title={tooltip}>
    {cards.map(card => <button type="button" key={card.key} className={activeFilter === card.status ? 'is-active' : ''} onClick={() => onFilter?.(activeFilter === card.status ? '' : card.status)}><span>{card.label}</span><b>{card.value}</b></button>)}
  </div>
}
...
function TaskRow({ task, selected, onOpen, onQuickAction, zh }) {
  ...
  return <div className={`autonomous-task-row${selected ? ' is-selected' : ''}`}>
    <button type="button" className="autonomous-task-row-main-button" onClick={() => onOpen(task)}>
      ...
      <span className="autonomous-task-row-meta"><span>{task.current_stage || statusLabel(task.status)}</span><span>{clamp(task.progress)}%</span></span>
...
function QuickCreate({ zh, onCreate, onEditFull }) {
  ...
  const submit = async () => {
    ...
    if (!confirmDanger('autonomous-task-create', zh ? `创建任务“${payload.title}”？` : `Create task "${payload.title}"?`)) return
    setPhase('creating')
...
export function AutonomousTaskWorkspace({ lang = 'zh' }) {
  const zh = lang !== 'en'
  const [tasks, setTasks] = useState([]); const [selected, setSelected] = useState(''); const [detail, setDetail] = useState(null); const [query, setQuery] = useState(''); const [status, setStatus] = useState(''); const [source, setSource] = useState(''); const [project, setProject] = useState(''); const [risk, setRisk] = useState(''); const [priority, setPriority] = useState(''); const [draft, setDraft] = useState(emptyDraft); const [editing, setEditing] = useState(''); const [editorOpen, setEditorOpen] = useState(false); const [loading, setLoading] = useState(false); const [saving, setSaving] = useState(false); const [message, setMessage] = useState(''); const [rejecting, setRejecting] = useState(null); const [rejectNote, setRejectNote] = useState('')
  const refresh = useCallback(async () => { setLoading(true); try { const params = new URLSearchParams(); if (query) params.set('q', query); if (status && status !== '__attention') params.set('status', status); if (source) params.set('source', source); if (project) params.set('project', project); if (risk) params.set('risk', risk); if (priority) params.set('priority', priority); const result = await api(`/api/autonomous/tasks${params.toString() ? `?${params}` : ''}`); let nextTasks = Array.isArray(result.tasks) ? result.tasks : []; if (status === '__attention') nextTasks = nextTasks.filter(task => ['blocked', 'paused', 'failed'].includes(task.status)); setTasks(nextTasks); if (selected && !nextTasks.some(task => task.id === selected)) { setSelected(''); setDetail(null) } } catch (error) { setMessage(error.message) } finally { setLoading(false) } }, [priority, project, query, risk, selected, source, status])
  ...
  const metrics = useMemo(() => taskMetrics(tasks), [tasks]); const sources = useMemo(() => [...new Set(tasks.map(task => task.source_type).filter(Boolean))], [tasks]); const projects = useMemo(() => [...new Set(tasks.map(task => task.project).filter(Boolean))], [tasks]); const risks = useMemo(() => [...new Set(tasks.map(task => task.risk).filter(Boolean))], [tasks]); const editTask = task => { setEditing(task.id); setEditorOpen(true); setDraft({ title: task.title || '', objective: task.objective || '', priority: task.priority || 'normal', risk: task.risk || '', project: task.project || '', due_at: task.due_at ? task.due_at.slice(0, 16) : '', next_step: task.next_step || '' }) }
