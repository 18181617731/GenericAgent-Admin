  ...
  return <>
    <div className="notification-center">
      ...
      {open && <div className="notification-popover" role="dialog" aria-label={copy.title}>
        ...
        <div className="notification-popover-list">{recent.length ? recent.map(item => <button type="button" key={item.id} className={itemClass(item)} onClick={() => openItem(item)}>
          <span className="notification-level-dot"/><span className="notification-item-copy"><strong>{item.title}</strong><small>{item.message}</small><em>{notificationCategoryLabel(item.category, lang)} · {formatNotificationTime(item.createdAt, lang)}</em></span><ExternalLink size={14}/>
        </button>) : <p className="notification-empty">{copy.empty}</p>}</div>
    ...
    {toast && <div className={`notification-toast is-${toast.level}`} role="status" aria-live="polite">
      <button type="button" className="notification-toast-main" onClick={() => openItem(toast)}><span className="notification-level-dot"/><span><strong>{toast.title}</strong><small>{toast.message}</small></span></button>
