    ...
    <article className={`model-config-row model-call-row${configOpen ? ' is-open is-expanded' : ''}${enabled ? '' : ' is-disabled'}`}>
      <div className="model-config-main">
        ...
        <Button
          ...
          aria-label={`配置: ${config.model || index + 1}`}
        >
        ...
        <div className="model-row-advanced">
          <div className="model-row-advanced-grid">
            <label className="model-field">
              ...
              <Input aria-label={text.displayName || '\u663e\u793a\u540d\u79f0'} value={displayName} onChange={event => onChange({ name: event.target.value })} placeholder={text.displayNamePlaceholder || '\u4f8b\u5982\uff1a\u4e3b\u6a21\u578b'} />
            </label>
            <label className="model-field">
...
function LegacyFailoverRows({ groups = [], text }) {
  const makeRows = source => migrateFailoverGroupNames(normalizeFailoverGroups(source)).map((group, index) => ({
    ...group,
    _compat_key: `compat-failover-${index}-${group.var_name || 'group'}`,
  }))
  const [rows, setRows] = useState(() => makeRows(groups))
  const [expanded, setExpanded] = useState(() => new Set())

  useEffect(() => {
    setRows(makeRows(groups))
    setExpanded(new Set())
  }, [groups])

  const addGroup = () => {
    const key = `compat-failover-new-${rows.length + 1}`
  ...
  return (
    <section className="model-call-list model-legacy-failover-summary" aria-label="故障转移组">
      <div className="model-call-head">
        <div>
          <strong>故障转移组</strong>
          <span>已保存策略按优先级排列，展开可查看成员。</span>
        </div>
        <Button size="small" onClick={addGroup} aria-label="新建故障转移组">新建故障转移组</Button>
      </div>
      <div className="model-call-rows">
        {rows.map((group, index) => {
          const isExpanded = expanded.has(group._compat_key)
          return (
            <article key={group._compat_key} className={`model-call-row is-failover${isExpanded ? ' is-expanded' : ''}`}>
              <div className="model-call-main">
                <div className="model-call-slot"><strong>{index}</strong></div>
                <div className="model-call-copy">
                  <div className="model-call-title"><strong>{group.var_name || `故障转移组 ${index + 1}`}</strong></div>
                  <div className="model-call-sub"><em>{group.members?.length || 0} 个成员</em></div>
                </div>
                <div className="model-call-actions">
                  <button
                    ...
                    onClick={() => setExpanded(current => {
                      ...
                      if (next.has(group._compat_key)) next.delete(group._compat_key)
                      else next.add(group._compat_key)
                      return next
                    })}
                  >
                    {isExpanded ? text.collapse : text.configure}
              ...
              {isExpanded && (
                <div className="model-row-body">
                  <strong>{text.failoverMembers || '成员'}</strong>
                  <span>{group.members?.length ? group.members.map(member => `${member.provider_var_name} · ${member.model}`).join('、') : text.failoverEmpty}</span>
                </div>
              )}
            </article>
          )
...
export function Models({
  ...
  const hasLegacyDraftBar = changes && typeof changes.total === 'number'
  const legacyCompatibility = typeof setFailoverGroups === 'function' && hasLegacyDraftBar
  const effectivePersistedProfiles = persistedProfiles.length ? persistedProfiles : profiles
  ...
  const persistedOrderCount = orderedModelRows(effectivePersistedProfiles).length
  ...
  const failoverCandidates = useMemo(() => orderedModelRows(effectivePersistedProfiles).map(row => {
    const protocol = String(effectivePersistedProfiles[row.profileIndex]?.type || DEFAULT_PROTOCOL)
    return { ...row, family: protocol.startsWith('native_') ? 'native' : 'legacy', protocol }
  }), [effectivePersistedProfiles])
  ...
  const openFailover = () => {
    const sourceGroups = failoverGroups.length ? failoverGroups : [{
      var_name: nextFailoverGroupName([]),
      members: [],
      max_retries: 10,
      base_delay: 0.5,
    }]
    setFailoverDrafts(migrateFailoverGroupNames(normalizeFailoverGroups(sourceGroups)).map(group => ({
      ...group,
  ...
  return (
    <section className="models-page">
      ...
      {hasLegacyDraftBar && (
        <div className="model-save-bar" role="status" aria-live="polite">
          <div className="model-save-summary">
            <span className={`model-summary-dot${changes.total > 0 ? ' is-dirty' : ''}${hasErrors ? ' is-error' : ''}`} />
            <strong>{changes.total > 0 ? `${changes.total} 处未保存改动` : '与 mykey.py 一致'}</strong>
            {saveState?.status === 'error' && <span className="is-error">存在保存错误</span>}
          </div>
          <div className="model-save-actions">
            <span className={`model-draft-state${changes.total > 0 ? ' is-dirty' : ''}`}>
              {changes.total > 0 ? '修改会在确认后写入 mykey.py' : '当前草稿没有需要写入的内容'}
            </span>
            <Button
              type="primary"
              disabled={!changes.total || !saveAll || saveState?.status === 'saving'}
