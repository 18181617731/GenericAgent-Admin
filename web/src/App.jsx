...
export default function App({ uiScale = 1, onUiScaleChange = () => {} }) {
  ...
  const notificationMonitorRef = useRef({ snapshot: null, busy: false })
  ...
  const nav = NAV_ITEMS
  const navGroups = NAV_GROUPS.map(group => ({ ...group, items: group.items.filter(item => nav.includes(item)) }))
  const navLabel = key => key === 'instances'
  ...
  const openNotification = item => {
    ...
    if (route === 'tasks' && item?.subtab) {
      navigateTaskSubTab(item.subtab)
      return
    }
    navigateTo(route || 'notifications')
  }
  ...
  return <>
    ...
    <main className="main"><header className="admin-page-header"><div className="admin-page-heading"><span className="admin-page-icon" aria-hidden="true">{icon(tab)}</span><div className="admin-page-copy"><h2>{navLabel(tab)}</h2><p>{pageDescription(tab)}</p></div></div><div className="admin-page-meta" aria-label={lang === 'zh' ? '服务状态' : 'Service status'}><span className="admin-page-endpoint"><Server size={14} aria-hidden="true"/><span>{cfg?.host}:{cfg?.port}</span></span><span role="status" aria-live="polite" className={`admin-page-health ${health?.ok ? 'is-ready' : 'is-error'}`}><span className="admin-page-health-dot" aria-hidden="true"/>{health?.ok ? t.ready : t.error}</span><NotificationCenter lang={lang} onOpen={openNotification}/></div></header>
...
export function ChannelsPage({ frontendSvcs, t, actionStates = {}, onStart, onStop, onLogs, onAutostart, onReflectStart, onOpenHub }) {
  ...
  const serviceFilters = [
    ['all', text.serviceFilterAll, allFrontendSvcs.length],
    ['running', text.serviceFilterRunning, runningCount],
    ['stopped', text.serviceFilterStopped, allFrontendSvcs.length - runningCount],
  ]
  const visibleFrontendSvcs = serviceFilter === 'running'
    ? allFrontendSvcs.filter(s => s.running)
  ...
  return <section className="channels-page">
    ...
    <div className="channels-layout">
      ...
      <section id="channel-panel-services" className="channel-services-view" role="tabpanel" aria-labelledby="channel-tab-services" hidden={activeView !== 'services'}>
