...
const I18N = {
  zh: {
    ...
    nav: { overview: '总览', chat: '对话', files: '文件', tasks: '定时任务', memory: '记忆', channels: '通道', autonomous: '自主进化', schedule: '定时', goals: 'Goal 模式', models: '模型', settings: '配置', logs: '日志' },
    desc: { overview: '从 GA 的功能域理解并接管生命周期。', chat: '迁移自 reactapp 的 GA 原生对话、文件上传和流式聊天界面。', files: '安全浏览 GA 根目录内文本文件，支持 tail 与搜索。', tasks: '管理 sche_tasks 定时任务、调度服务和执行记录。', memory: '分层记忆、SOP 与工具能力索引。', channels: '桌面、TUI、Web、IM Bot 等前端入口。', autonomous: '管理反思与自主运行服务，查看自主进化报告。', schedule: 'sche_tasks JSON 定时任务详情、编辑、创建与删除。', goals: '复用 GA Goal Mode SOP 与 reflect/goal_mode.py 的持续目标控制台。', models: '按服务商读取、预览和保存 GA mykey.py 中的模型配置。', settings: '配置 GA 根目录、Python、聊天数据目录与 Chat Python 代理。', logs: '进程状态与输出日志。' },
...
export default function App() {
  ...
  const setMsg = (message, kind) => {
    ...
    const inferredKind = /(?:失败|错误|无效|error|failed|invalid)/i.test(value)
      ? 'error'
  ...
  const discardFileChanges = () => { setFileContent(loadedFileContent); setFilePath(loadedFilePath); setMsg('已放弃未保存的文件更改') }
  const refreshApp = () => {
    ...
    <main className="main"><header><div><h2>{t.nav[tab]}</h2><p>{t.desc[tab]}</p></div><div className="badges"><span>{cfg?.host}:{cfg?.port}</span><span role="status" aria-live="polite" className={health?.ok?'ok':'err'}>{health?.ok ? t.ready : t.error}</span></div></header>
      ...
      {tab==='tasks' && <section className="tasks-page">
        ...
        {taskSubTab==='scheduled' && <>
          <div className="single-panel schedule-service-panel">
