...
import { Activity, Bot, Brain, BarChart3, CalendarClock, CheckCircle2, ClipboardList, Code2, Copy, Eye, ExternalLink, FileCode2, FolderCog, Globe2, GitPullRequest, Menu, MessageSquare, PanelLeftClose, Play, RefreshCw, Save, Server, ShieldAlert, Power, SlidersHorizontal, Square, Sparkles, Target, Terminal, Trash2, UploadCloud, XCircle, Download } from 'lucide-react'
...
export const I18N = {
  zh: {
    ...
    nav: { overview: '总览', instances: 'GA 实例', chat: '对话', files: '文件', tasks: '任务', memory: '记忆', channels: '通道', autonomous: '自主进化', usage: '用量总览', workboard: '工作看板', schedule: '定时', goals: 'Goal 模式', models: '模型', settings: '配置', logs: '日志' },
    desc: { overview: '从 GA 的功能域理解并接管生命周期。', instances: '管理隔离的 GA 运行目录、Python 解释器和默认实例。', chat: '迁移自 reactapp 的 GA 原生对话、文件上传和流式聊天界面。', files: '安全浏览 GA 根目录内文本文件，支持 tail 与搜索。', tasks: '普通会话、任务文件、批处理入口、任务型服务与 sche_tasks 定时任务。', memory: '分层记忆、SOP 与工具能力索引。', channels: '桌面、TUI、Web、IM Bot 等前端入口。', autonomous: '反思、自主运行、Goal Mode 与团队 Worker。', workboard: '带负责人、风险标签与审批门禁的持久化工作流。', schedule: 'sche_tasks JSON 定时任务详情、编辑、创建与删除。', goals: '复用 GA Goal Mode SOP 与 reflect/goal_mode.py 的持续目标控制台。', models: '按服务商读取、预览和保存 GA mykey.py 中的模型配置。', settings: '配置 GA 根目录、Python、聊天数据目录与 Chat Python 代理。', logs: '进程状态与输出日志。' },
    ...
    models: {
      ...
      blockItems: n => `${n} 个阻断项`, reminders: n => `${n} 个提醒`, valid: '配置有效', inherit: '使用默认值', defaultParams: '默认参数', unnamedModel: '未命名模型', displayName: '显示名称', displayNamePlaceholder: '不填则显示模型 ID', collapse: '收起', configure: '配置', deleteModel: name => `删除模型 ${name}`, stream: '流式输出', maxRetries: '最大重试次数', readTimeout: '读取超时（秒）', connectTimeout: '连接超时（秒）', optional: '可选', apiMode: 'API 模式', thinkingType: '思考类型', reasoningEffort: '推理强度', fakeClaude: '模拟 Claude Code 系统提示',
      ...
      addProvider: '新增服务商', addProviderHelp: '一个服务商可同时管理多个模型，只需配置一次协议、BaseURL 和密钥。', closeAdd: '关闭新增面板', nameRequired: '请填写名称。', baseRequired: '请填写 BaseURL。', optionalKey: '可选', addProviderFooter: '添加后会立即保存到 mykey.py；后续修改仍需在卡片中点击“保存”。', addAndSave: '添加并保存', rereadConfig: '重新读取', modelOrder: '模型顺序', failover: '故障转移', failoverAvailable: '配置已保存模型的故障转移链', failoverTitle: '故障转移策略', failoverSaveMissing: '当前页面未提供故障转移保存能力，请刷新后重试。', failoverSaveFailed: '保存失败，当前策略草稿已保留。', failoverDiscard: '关闭会丢弃未保存的策略调整', failoverInfo: '显式保存的全局策略', failoverDescription: '选择同一协议族的模型并设定优先级。只有点击“确认并保存”后才会写入 mykey.py。', failoverNeedsTwo: '故障转移链至少需要 2 个模型；也可移除全部成员以清空策略。', failoverSameFamily: '一条故障转移链不能混用 Native 与 Legacy 协议族。', failoverRetriesInvalid: '最大重试次数必须是大于或等于 0 的整数。', failoverDelayInvalid: '基础延迟必须是大于或等于 0 的数字。', failoverSpringInvalid: '自动回弹秒数留空或填写大于 0 的整数。', failoverMembers: '选择成员', failoverSelected: n => `已选 ${n} 个`, failoverMembersHelp: '首次选择会锁定协议族，其他协议族将不可选。', failoverNative: 'Native 协议族', failoverNativeHelp: '官方 OpenAI / Claude / Gemini 原生协议', failoverLegacy: 'Legacy 协议族', failoverLegacyHelp: '兼容和历史协议适配器', failoverLocked: '已锁定其他协议族', failoverNoCandidates: '该协议族暂无已保存模型。', failoverPriority: '设定优先级', failoverPriorityHelp: '编号 0 为首选模型，失败后依次尝试后续模型。可拖拽或使用箭头调整。', failoverRemove: '从故障转移链移除', failoverEmpty: '尚未选择模型。保存空链将清除已有故障转移策略。', failoverPolicy: '配置重试策略', failoverPolicyHelp: '设置每个候选模型的重试和延迟行为。', failoverRetries: '最大重试次数', failoverRetriesHelp: '每个模型失败前的重试次数，默认 10。', failoverDelay: '基础延迟（秒）', failoverDelayHelp: '重试之间的基础等待时间，默认 0.5 秒。', failoverSpring: '自动回弹（秒）', failoverSpringPlaceholder: '留空表示不启用', failoverSpringHelp: '切换到备用模型后，间隔指定秒数再尝试首选模型。', orderTitle: '调整模型顺序', orderAvailable: '调整已保存模型的全局顺序', orderUnavailable: '保存模型后才能调整顺序', configPreview: '配置预览', configSummary: '配置概况', providers: n => `${n} 个服务商`, models: n => `${n} 个模型`, configSource: '配置来源', pageHasErrors: '存在不能保存的服务商，请在目录中选择异常项并修复。', providerDirectory: '服务商目录', chooseProvider: '选择一项进行编辑', providerNav: '模型服务商', stateError: '存在阻断项', stateWarning: '存在提醒', stateReady: '配置正常', singleSaveHint: '单项保存，密钥不会出现在预览中', providerEditor: '服务商编辑器', loadingMykey: '正在读取 mykey.py…', noProviders: '还没有服务商', loadingHelp: '请稍候，配置加载后会显示在这里。', noProvidersHelp: '创建第一个服务商，将同源模型集中管理。',
  ...
  en: {
    ...
    nav: { overview: 'Overview', instances: 'GA Instances', chat: 'Chat', files: 'Files', tasks: 'Tasks', memory: 'Memory', channels: 'Channels', autonomous: 'Autonomous', usage: 'Usage', workboard: 'Workboard', schedule: 'Schedule', goals: 'Hive Mode', models: 'Models', settings: 'Settings', logs: 'Logs' },
    desc: { overview: 'Understand and take over GA lifecycle by native domains.', instances: 'Manage isolated GA runtime roots, Python interpreters, and the default instance.', chat: 'GA native conversation, uploads and streaming UI migrated from reactapp.', files: 'Safely browse text files inside GA root with tail and search.', tasks: 'Conversations, task files, batch entrypoints and task services.', memory: 'Layered memory, SOPs and utility indexes.', channels: 'Desktop, TUI, Web and IM Bot entrypoints.', autonomous: 'Reflection, autonomous runs, Goal Mode and team workers.', workboard: 'A persistent owned and risk-labelled workflow with reviewed transitions.', schedule: 'View, edit, create and delete sche_tasks JSON jobs.', goals: 'Continuous objective control console backed by GA Goal Mode SOP and reflect/goal_mode.py.', models: 'Import, preview and write GA mykey.py model config.', settings: 'Configure GA root, Python, chat data directory, and Chat Python proxy.', logs: 'Process state and output logs.' },
    ...
    models: {
      ...
      blockItems: n => `${n} blocking item${n === 1 ? '' : 's'}`, reminders: n => `${n} reminder${n === 1 ? '' : 's'}`, valid: 'Configuration valid', inherit: 'Use default', defaultParams: 'Default parameters', unnamedModel: 'Unnamed model', displayName: 'Display name', displayNamePlaceholder: 'Leave blank to show the model ID', collapse: 'Collapse', configure: 'Configure', deleteModel: name => `Delete model ${name}`, stream: 'Streaming output', maxRetries: 'Maximum retries', readTimeout: 'Read timeout (seconds)', connectTimeout: 'Connection timeout (seconds)', optional: 'Optional', apiMode: 'API mode', thinkingType: 'Thinking type', reasoningEffort: 'Reasoning effort', fakeClaude: 'Emulate Claude Code system prompt',
      ...
      addProvider: 'Add provider', addProviderHelp: 'One provider can manage multiple models with one protocol, BaseURL, and key.', closeAdd: 'Close add-provider panel', nameRequired: 'Enter a name.', baseRequired: 'Enter a BaseURL.', optionalKey: 'Optional', addProviderFooter: 'The provider is written to mykey.py immediately. Later edits still require Save in its card.', addAndSave: 'Add and save', rereadConfig: 'Reload', modelOrder: 'Model order', failover: 'Failover', failoverAvailable: 'Configure failover for saved models', failoverTitle: 'Failover policy', failoverSaveMissing: 'Failover saving is unavailable on this page. Refresh and try again.', failoverSaveFailed: 'Save failed. The current policy draft has been retained.', failoverDiscard: 'Closing discards unsaved policy changes', failoverInfo: 'Explicitly saved global policy', failoverDescription: 'Select models from one protocol family and set their priority. Nothing is written to mykey.py until you select Confirm and save.', failoverNeedsTwo: 'A failover chain needs at least 2 models. Remove every member to clear the policy instead.', failoverSameFamily: 'A failover chain cannot mix Native and Legacy protocol families.', failoverRetriesInvalid: 'Maximum retries must be an integer greater than or equal to 0.', failoverDelayInvalid: 'Base delay must be a number greater than or equal to 0.', failoverSpringInvalid: 'Spring-back seconds must be blank or an integer greater than 0.', failoverMembers: 'Select members', failoverSelected: n => `${n} selected`, failoverMembersHelp: 'Your first selection locks the protocol family and disables the other family.', failoverNative: 'Native family', failoverNativeHelp: 'Official OpenAI, Claude, and Gemini native protocols', failoverLegacy: 'Legacy family', failoverLegacyHelp: 'Compatibility and historical protocol adapters', failoverLocked: 'Another family is active', failoverNoCandidates: 'No saved models are available in this family.', failoverPriority: 'Set priority', failoverPriorityHelp: 'Index 0 is preferred. Failures advance through later models. Drag rows or use the arrow controls.', failoverRemove: 'Remove from failover chain', failoverEmpty: 'No models selected. Saving an empty chain clears the existing failover policy.', failoverPolicy: 'Configure retry policy', failoverPolicyHelp: 'Set retry and delay behavior for every candidate model.', failoverRetries: 'Maximum retries', failoverRetriesHelp: 'Retries before leaving each model. Default: 10.', failoverDelay: 'Base delay (seconds)', failoverDelayHelp: 'Base wait between retries. Default: 0.5 seconds.', failoverSpring: 'Spring back (seconds)', failoverSpringPlaceholder: 'Blank to disable', failoverSpringHelp: 'After switching to a fallback, wait this long before trying the preferred model again.', orderTitle: 'Adjust model order', orderAvailable: 'Adjust the global order of saved models', orderUnavailable: 'Save models before adjusting their order', configPreview: 'Configuration preview', configSummary: 'Configuration summary', providers: n => `${n} provider${n === 1 ? '' : 's'}`, models: n => `${n} model${n === 1 ? '' : 's'}`, configSource: 'Configuration source', pageHasErrors: 'Some providers cannot be saved. Select each flagged item and resolve it.', providerDirectory: 'Provider directory', chooseProvider: 'Select an item to edit', providerNav: 'Model providers', stateError: 'Has blocking items', stateWarning: 'Has reminders', stateReady: 'Configuration ready', singleSaveHint: 'Save providers individually; keys never appear in previews', providerEditor: 'Provider editor', loadingMykey: 'Loading mykey.py…', noProviders: 'No providers yet', loadingHelp: 'Please wait; configuration will appear here after loading.', noProvidersHelp: 'Create the first provider to manage models from the same source together.',
...
export default function App() {
  ...
  return <>
    ...
    <main className="main">
      ...
      {tab==='workboard' && <WorkboardPage lang={lang}/>}
...
function icon(n) { const m = { overview:<Activity size={16}/>, instances:<Server size={16}/>, chat:<MessageSquare size={16}/>, files:<FileCode2 size={16}/>, tasks:<Terminal size={16}/>, memory:<Brain size={16}/>, channels:<Globe2 size={16}/>, autonomous:<Bot size={16}/>, usage:<BarChart3 size={16}/>, workboard:<ClipboardList size={16}/>, schedule:<CalendarClock size={16}/>, goals:<Target size={16}/>, models:<SlidersHorizontal size={16}/>, settings:<FolderCog size={16}/>, logs:<FolderCog size={16}/> }; return m[n] }
