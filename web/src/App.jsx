...
export const I18N = {
  zh: {
    ...
    nav: { overview: '总览', instances: 'GA 实例', chat: '对话', files: '文件', tasks: '任务', memory: '记忆', channels: '通道', autonomous: '自主进化', usage: '用量总览', schedule: '定时', goals: 'Goal 模式', models: '模型', settings: '配置', logs: '日志' },
    ...
    models: {
      ...
      blockItems: n => `${n} 个阻断项`, reminders: n => `${n} 个提醒`, valid: '配置有效', inherit: '使用默认值', defaultParams: '默认参数', unnamedModel: '未命名模型', displayName: '显示名称', displayNamePlaceholder: '不填则显示模型 ID', collapse: '收起', configure: '配置', deleteModel: name => `删除模型 ${name}`, stream: '流式输出', maxRetries: '最大重试次数', readTimeout: '读取超时（秒）', connectTimeout: '连接超时（秒）', optional: '可选', apiMode: 'API 模式', serviceTier: '服务层级', thinkingType: '思考类型', reasoningEffort: '推理强度', fakeClaude: '模拟 Claude Code 系统提示',
  ...
  en: {
    ...
    nav: { overview: 'Overview', instances: 'GA Instances', chat: 'Chat', files: 'Files', tasks: 'Tasks', memory: 'Memory', channels: 'Channels', autonomous: 'Autonomous', usage: 'Usage', schedule: 'Schedule', goals: 'Hive Mode', models: 'Models', settings: 'Settings', logs: 'Logs' },
    ...
    models: {
      ...
      blockItems: n => `${n} blocking item${n === 1 ? '' : 's'}`, reminders: n => `${n} reminder${n === 1 ? '' : 's'}`, valid: 'Configuration valid', inherit: 'Use default', defaultParams: 'Default parameters', unnamedModel: 'Unnamed model', displayName: 'Display name', displayNamePlaceholder: 'Leave blank to show the model ID', collapse: 'Collapse', configure: 'Configure', deleteModel: name => `Delete model ${name}`, stream: 'Streaming output', maxRetries: 'Maximum retries', readTimeout: 'Read timeout (seconds)', connectTimeout: 'Connection timeout (seconds)', optional: 'Optional', apiMode: 'API mode', serviceTier: 'Service tier', thinkingType: 'Thinking type', reasoningEffort: 'Reasoning effort', fakeClaude: 'Emulate Claude Code system prompt',
