...
import { NAV_ITEMS, TASK_SUB_TABS, parseRoute } from './lib/routing'
...
export default function App() {
  ...
  return <>
    ...
    <aside id="admin-sidebar" className="sidebar">
      ...
      <nav aria-label={t.mainNavigation}>{nav.map(n => <button key={n} type="button" aria-current={tab===n ? 'page' : undefined} className={tab===n?'active':''} onClick={()=>{ setAdminSidebarOpen(false); if (n === 'models') openModels(); else setTab(n) }}>{icon(n)}{t.nav[n]}</button>)}</nav>
