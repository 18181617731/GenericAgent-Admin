...
export default function App() {
  ...
  const [profiles, setProfiles] = useState([]), [modelPreview, setModelPreview] = useState(''), [revealedKeys, setRevealedKeys] = useState({})
  ...
  const revealModels = async () => { try { const d = await api('/api/models/raw'); const m = {}; (d.profiles||[]).forEach(p => { if (p.var_name && p.apikey) m[p.var_name] = p.apikey }); setRevealedKeys(m); setProfiles(ps => ps.map(p => m[p.var_name] ? { ...p, apikey: m[p.var_name] } : p)) } catch(e){ setMsg(e.message) } }
  ...
  return <div ref={appScope} className={`app app-tab-${tab}`}>
    ...
    <main className="main"><header><div><h2>{t.nav[tab]}</h2><p>{t.desc[tab]}</p></div><div className="badges"><span>{cfg?.host}:{cfg?.port}</span><span role="status" aria-live="polite" className={health?.ok?'ok':'err'}>{health?.ok ? t.ready : t.error}</span></div></header>
      ...
      {tab==='files' && <FilesPage t={t} filePath={filePath} setFilePath={setFilePath} fileList={fileList} fileContent={fileContent} setFileContent={setFileContent} fileSearch={fileSearch} setFileSearch={setFileSearch} searchHits={searchHits} tailLines={tailLines} setTailLines={setTailLines} loadFiles={loadFiles} readFile={readFile} tailFile={tailFile} saveFile={saveFile} runSearch={runSearch}/>}
      ...
      {tab==='models' && <Models t={t} profiles={profiles} setProfiles={setProfiles} patchProfile={patchProfile} importModels={importModels} previewModels={previewModels} saveModels={saveModels} modelPreview={modelPreview} revealModels={revealModels} revealedKeys={revealedKeys}/>} 
