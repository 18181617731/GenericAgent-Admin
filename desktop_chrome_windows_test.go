...
func TestShippedIconHasAFrameForEveryScaledWindowSize(t *testing.T) {
	sides := map[int]int{}
	for i := 0; i < int(binary.LittleEndian.Uint16(appIconICO[4:])); i++ {
		at := 6 + i*16
		sides[int(binary.LittleEndian.Uint32(appIconICO[at+12:]))] = int(appIconICO[at])
	}
	for _, want := range []int{16, 20, 24, 28, 32, 36, 40, 48, 56, 64, 72, 80, 96} {
		offset, length := bestICOFrame(appIconICO, want, want)
		if length == 0 {
			t.Fatalf("%dpx: no usable frame in assets/tray_windows.ico", want)
		}
