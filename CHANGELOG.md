...
### 定时任务模型与文件夹
- 定时任务可跟随调度器默认模型，也可为单个任务选择模型；以模型、服务商和 BaseURL 组成的稳定标识保存，模型列表重排不会静默改变任务目标。旧序号任务在保存时迁移，已失效模型会阻止执行并提示重新选择。
- 新增全部、未归类和自定义文件夹筛选与任务卡片归类；支持新建、重命名、删除文件夹，删除文件夹只解除归类，不删除任务。文件夹元数据原子写入并保留可恢复备份。
