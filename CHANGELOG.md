...
### 移动聊天侧栏布局
- 移动端聊天侧栏改为单一纵向滚动容器，保留顶部关闭按钮并让全部项目、会话和底部操作可达，避免控件占满视口后列表被截断。
- 修复侧栏打开时回调变化触发焦点回顶，确保滚动位置不会因刷新或状态更新被重置。
...
### 移动端页面布局收口
- 修复手机端滚动时页面标题卡片被顶部栏吸附遮挡的问题，长描述完整换行显示。
- 将通道配置从横向半截卡片改为两列自适应列表，报告、任务和实例路径在窄屏下换行展示，避免关键信息被省略。
...
### 移动端页面可用性
- 修复模型工作台、模型预览和新增服务商面板在手机宽度下的横向溢出与内容挤压。
- 为通道配置/服务管理补齐移动端布局，保证字段、操作和服务卡片在视口内可读。
- 让管理导航抽屉打开时锁定页面滚动，并避免长页面标题覆盖移动端顶部栏。
- 允许定时任务最近执行结果在手机端换行显示，避免关键信息被单行裁切。

## v1.0.113 - 2026-09-02

### 定时任务状态提示
- 为任务卡片中的“执行结果”和“配置状态”增加明确前缀，避免最近一次执行成功与当前计划逾期/异常被误读为互相矛盾。
- 在定时任务工作台补充状态说明，并让旧版任务卡片沿用同一套独立状态语义。

## v1.0.112 - 2026-09-02

### 前端工作台稳定性
- 修复 Mermaid 全屏按钮在合并后引用不存在图标导致的构建问题。
- 保留用户消息中的非图片附件（包括重新加载后的文件元数据），并为模型工具按钮补充无障碍名称。
- 将模型工作台冒烟覆盖与当前服务商侧栏和内联模型编辑器契约对齐，避免发布验证误报。
...
### GA 实例创建体验
- 将从已有实例克隆改为后台任务，创建后立即显示排队、扫描、复制、校验和收尾进度，不再让页面长时间无反馈。
- 持久化克隆来源与复制选项，服务重启后可继续未完成的克隆；失败时保留实例卡片、错误详情和可恢复的空根目录。
- 跳过 Rust `target` 等生成构建产物，避免源项目缓存超过 2 GiB 安全上限导致默认实例克隆失败。
