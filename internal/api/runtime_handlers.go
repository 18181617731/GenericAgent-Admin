...
func (s *Server) gaRuntimeRepair(w http.ResponseWriter, r *http.Request) {
	...
	python, ready := s.ensureManagedRuntimePython(r.Context(), root, python, cfg, &result)
	if !ready {
	...
	result.After = buildRuntimeHealthForRepair(root, python)
	s.persistRuntimePython(r, root, &result)
...
func (s *Server) persistRuntimePython(r *http.Request, root string, result *runtimeRepairResult) {
	if result.After.Runtime != nil && isAbsoluteExistingFile(result.After.Runtime.PythonPath) {
		...
		cfg, err := s.configViewForRequest(r)
		if err != nil {
			result.Errors = append(result.Errors, "读取当前实例配置失败: "+err.Error())
			return
		}
		if cfg.PythonPath != result.Python {
			...
			if _, saveErr := s.saveSetupConfig(r, cfg); saveErr != nil {
...
func (s *Server) ensureManagedRuntimePython(ctx context.Context, root, python string, cfg config.AppConfig, result *runtimeRepairResult) (string, bool) {
	managed := setupVenvPython(root)
	configured := strings.TrimSpace(cfg.EffectivePython)
	if isAbsoluteExistingFile(managed) || isAbsoluteExistingFile(configured) {
		return python, true
	}
	base, err := runtimeRepairExecutablePath(pythonForSetup(root, cfg))
	if err != nil {
