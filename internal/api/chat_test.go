...
func TestChatPythonForConfigPrefersConfiguredPythonPath(t *testing.T) {
	...
	if err := os.WriteFile(configured, []byte("fake"), 0755); err != nil {
		t.Fatal(err)
	}
...
func TestChatPythonForConfigFallsBackWhenConfiguredPythonWasDeleted(t *testing.T) {
	root := t.TempDir()
	venvDir := filepath.Join(root, ".venv", "bin")
	venvPython := filepath.Join(venvDir, "python")
	if runtime.GOOS == "windows" {
	...
	got := chatPythonForConfig(config.AppConfig{GARoot: root, PythonPath: filepath.Join(root, "temp", ".venv", "python")})
	if got != venvPython {
		t.Fatalf("chatPythonForConfig()=%q want managed venv %q", got, venvPython)
	}
}
