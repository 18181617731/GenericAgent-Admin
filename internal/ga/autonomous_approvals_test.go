...
func TestBuildAutonomousApprovalsParsesTrackedDrafts(t *testing.T) {
	...
	if first.Title != "daily_git_conflict_sop_DRAFT" || first.State != "pending" || first.Target != "memory/daily_git_conflict_sop.md" || first.ExpectedOutcome != "以后遇到同类问题可以直接按这份方案处理" || first.ReviewStatus != "fallback" || first.ReviewDecision != "needs_approval" || first.ReviewModelNo != 12 || first.ReviewModel != "gpt-5.6-luna" {
