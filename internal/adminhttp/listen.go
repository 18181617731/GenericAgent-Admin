// Package adminhttp binds the admin server's socket, records where it ended
// up, and builds the HTTP server that runs on it.
package adminhttp
...
// NewServer builds the admin HTTP server. The timeouts are set here so that no
// launch path can produce a server without them.
...
// OpenListener binds the admin HTTP listener for this launch.
//
...
func OpenListener(cfg config.AppConfig, portOverride int, passwordConfigured bool) (net.Listener, error) {
...
// WriteRuntimeInfo records where this process ended up listening.
