...
const (
	...
	legacyEncodingHeader = "# -*- coding: utf-8 -*-"
...
func stripLegacyRenderedModels(content string) string {
	firstLine, remaining := splitSourceLine(content)
	firstLine = strings.TrimPrefix(firstLine, "\ufeff")
	if firstLine == legacyEncodingHeader {
		firstLine, _ = splitSourceLine(remaining)
	}
...
func splitSourceLine(content string) (string, string) {
	end := strings.IndexByte(content, '\n')
	if end < 0 {
		return strings.TrimSuffix(content, "\r"), ""
	}
	return strings.TrimSuffix(content[:end], "\r"), content[end+1:]
}
