// Package tray runs the system tray menu: the one surface the OS draws for
// this app, and the only place a user can read the address of a server that
// listens on a random port by default.
package tray
...
// App is what the tray can do on behalf of the running admin server, and where
...
type App struct {
	...
	// ListenAddr is the address the server really bound. It cannot change
	// while the process runs.
	ListenAddr string
	// Config, PasswordConfigured and RunningServices are polled while the
	// process runs; all three must be cheap and safe to call from the tray
	// goroutine.
...
// status renders the menu text for the server as it stands right now. An App
// with no config source has nothing to report and says so with a zero value
// rather than describing an address it cannot place in context.
func (a App) status() menuStatus {
	if a.Config == nil {
		return menuStatus{}
	}
	passwordSet := a.PasswordConfigured != nil && a.PasswordConfigured()
	return describeTrayStatus(a.ListenAddr, a.Config(), passwordSet, primaryLANAddress, trayLanguage())
}

func (a App) runningServices() int {
...
// menuStatus is everything the tray menu says about where this process can be
...
func describeTrayStatus(listenAddr string, cfg config.AppConfig, passwordSet bool, lan func() string, text trayText) menuStatus {
