// Package desktop opens the admin and chat UIs, either in a native webview
// window or, wherever that is unavailable, in the system browser.
package desktop
...
// windows entirely and sends every open to the system browser.
func NewUI(appRoot string, browserOnly bool) *UI {
	if browserOnly {
		return &UI{}
	}
	dataPath := filepath.Join(appRoot, "data", "webview")
	return &UI{windows: newDesktopWindows(dataPath, openBrowser)}
}

// OpenChat opens the primary interface, which the web bundle serves at the
// root path.
func (u *UI) OpenChat(baseURL string) {
...
func (u *UI) OpenSettings(baseURL string) {
	u.open(desktopWindowSpec{
...
func (u *UI) open(spec desktopWindowSpec) {
	if u == nil {
		return
	}
	if u.windows == nil {
